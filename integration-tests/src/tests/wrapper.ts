...
async function testWrapperSuccessSwap(
  ...
  const beforeTcw: any = await client.getBalance(
  ...
  await wrapperClient.swap(
    ...
    "auto",
    undefined,
    [coin(uosmo_amount, "uosmo")]
  );

  const afterTcw: any = await client.getBalance(
    localosmosis.addresses.wallet2,
    out_denom
  );
...
async function testWrapperWholeFlow(
  ...
  await registryClient.createRequest(
    ...
    "auto",
    undefined,
    [coin(parseInt(uosmo_amount) + parseInt("1000"), "uosmo")]
  );
  ...
  const beforeTcw = await client.getBalance(
    localosmosis.addresses.wallet2,
    out_denom
  );
  const beoreTcwBalance = beforeTcw.amount;
...
async function walletAddLiquidity(
  ...
  const res = await sender_client.execute(
    senderAddress,
    tcw,
    {
      increase_allowance: {
  ...
  // Add liquidity
  const result = await sender_client.execute(
    senderAddress,
    swap,
    {
      add_liquidity: {
